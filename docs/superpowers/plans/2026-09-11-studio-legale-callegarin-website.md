...
3. Implement an in-memory token bucket keyed by a privacy-preserving truncated/hash representation of the client address. Bound the map and expire idle buckets on access; trust forwarded headers only when `TRUSTED_PROXY_HOPS=1` and remove exactly that trusted ingress hop from the right.
...
- A documented idempotent bootstrap creates the Entra application/service principal, a federated credential restricted to GitHub's immutable production-environment subject `repo:francescostumpo@55147498/legal-callegarin@1365534753:environment:production`, and only the built-in Container Apps Contributor role (`358470bc-b998-42bd-ab17-a7e34c199c0f`) on the one resource group. The numeric IDs come from the repository's public GitHub metadata and the bootstrap fails closed instead of using the legacy name-only subject.
...
3. Add `scripts/bootstrap-github-oidc.sh` with `--dry-run` and explicit required inputs, including the verified immutable GitHub owner/repository IDs. It creates or reuses the Entra app/service principal, configures the exact immutable production-environment federated subject and Azure token-exchange audience, and assigns Container Apps Contributor at the existing resource-group scope. Test argument validation/rendered intent without calling Azure. Reject the legacy name-only subject and document that only an authorized operator may execute the mutating mode.
