...
if docker container inspect "$SMOKE_CONTAINER" >/dev/null 2>&1; then
  echo "smoke container already exists: $SMOKE_CONTAINER" >&2
  exit 1
fi
...
docker build --build-arg VERSION=local-test --build-arg COMMIT=9084831 -t "$SMOKE_IMAGE" .
...
for attempt in $(seq 1 60); do
  if curl -fsS "$SMOKE_ORIGIN/health/live" >/dev/null; then
    ready=true
    break
  fi
  sleep 0.25
...
curl -fsS "$SMOKE_ORIGIN/health/ready" | grep -qx 'ok'
curl -fsS -D "$SMOKE_DIR/public.headers" -o "$SMOKE_DIR/public.html" "$SMOKE_ORIGIN/"
...
test -n "$public_asset"
curl -fsS "$SMOKE_ORIGIN$public_asset" >/dev/null

curl -fsS -o "$SMOKE_DIR/login.html" "$SMOKE_ORIGIN/admin/login"
...
curl -fsS -H "Cookie: $session_cookie" -o "$SMOKE_DIR/admin.html" "$SMOKE_ORIGIN/admin"
admin_asset=$(grep -o '/admin/assets/index-[A-Za-z0-9_-]*\.js' "$SMOKE_DIR/admin.html" | sed -n '1p')
test -n "$admin_asset"
curl -fsS -H "Cookie: $session_cookie" "$SMOKE_ORIGIN$admin_asset" >/dev/null

api_status=$(curl -sS -o "$SMOKE_DIR/api.json" -w '%{http_code}' "$SMOKE_ORIGIN/api/admin/session")
test "$api_status" = 401
...
docker stop -t 25 "$SMOKE_CONTAINER" >/dev/null
stop_finished_at=$(date +%s%3N)
echo "graceful_stop_ms=$((stop_finished_at - stop_started_at))"

exit_code=$(docker inspect --format '{{.State.ExitCode}}' "$SMOKE_CONTAINER")
oom_killed=$(docker inspect --format '{{.State.OOMKilled}}' "$SMOKE_CONTAINER")
test "$exit_code" = 0
