...
function parseRfc3339Nanos(value, label = "timestamp", nowNanos) {
  ...
  if (
    localDate.getUTCFullYear() !== y ||
    localDate.getUTCMonth() !== mo - 1 ||
    localDate.getUTCDate() !== d ||
    localDate.getUTCHours() !== h ||
    localDate.getUTCMinutes() !== mi ||
  ...
  if (zone !== "Z") {
    ...
    offsetMinutes =
      (zone[0] === "+" ? 1 : -1) * (offsetHours * 60 + offsetRemainder)
  }
  const fractionNanos = BigInt((fraction + "000000000").slice(0, 9))
  const instant =
    BigInt(localDate.getTime()) * 1_000_000n +
    fractionNanos -
    BigInt(offsetMinutes) * 60_000_000_000n
  if (nowNanos !== undefined && instant > nowNanos)
    fail(`${label} is invalid or in the future`)
  return instant
}
...
const compareNewest = (left, right) =>
  compareDescending(
    parseRfc3339Nanos(left.created_at),
    parseRfc3339Nanos(right.created_at),
  ) ||
  compareDescending(
    parseRfc3339Nanos(left.updated_at),
    parseRfc3339Nanos(right.updated_at),
  ) ||
  right.id - left.id
...
export function selectRetentionCandidates(
  ...
  const newestIds = new Set(
    ordered.slice(0, RETAIN_NEWEST).map((version) => version.id),
  )
...
function parseArguments(argv, environment) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (
      ...
      fail("argument value is missing")
  ...
  if (!subscriptionId || !resourceGroup || !containerAppName || !mode)
  ...
  if (
    ...
    fail("subscription ID must be a canonical lowercase UUID")
  if (
    resourceGroup.length > 90 ||
    !(
      /^[A-Za-z0-9]$/.test(resourceGroup) ||
      /^[A-Za-z0-9][A-Za-z0-9._-]*[A-Za-z0-9_]$/.test(resourceGroup)
    )
  )
    fail("invalid resource group name")
  if (
    containerAppName.length < 2 ||
    containerAppName.length > 31 ||
    !/^[a-z][a-z0-9-]*[a-z0-9]$/.test(containerAppName) ||
    containerAppName.includes("--")
  )
    fail("invalid Container App name")
  if (
...
function validateAccountAndGroup(config, environment) {
  const account = parseJson(
    executeCommand(
      "az",
      ["account", "show", "--query", "{id:id,state:state}", "--output", "json"],
      "Azure account read",
      environment,
    ),
    "Azure account read",
  ...
  if (
    ...
    account.id.toLowerCase() !== config.subscriptionId ||
    account.state !== "Enabled"
  )
    fail("Azure account is not the requested enabled subscription")
  ...
  const actual = executeCommand(
    ...
    "Azure resource group read",
    environment,
  ).trim()
  if (!actual || actual.toLowerCase() !== expected.toLowerCase())
    fail("Azure resource group identity mismatch")
}

function validateTraffic(traffic) {
  if (!Array.isArray(traffic) || traffic.length === 0)
    fail("Container App traffic is malformed")
  let total = 0
  const positive = []
  const revisions = new Set()
  for (const rawRule of traffic) {
    const rule = object(rawRule, "traffic rule")
    if (own(rule, "latestRevision") || own(rule, "label"))
      fail("traffic must not use latestRevision or labels")
    if (
      typeof rule.revisionName !== "string" ||
      !safeRevisionPattern.test(rule.revisionName)
    )
      fail("traffic revision name is missing or unsafe")
    if (revisions.has(rule.revisionName))
      fail("traffic contains duplicate revision names")
    revisions.add(rule.revisionName)
    if (!Number.isInteger(rule.weight) || rule.weight < 0 || rule.weight > 100)
      fail("traffic weight is invalid")
    total += rule.weight
    if (rule.weight > 0) positive.push(rule)
  }
  if (total !== 100 || positive.length !== 1 || positive[0].weight !== 100)
    fail("traffic must route exactly 100 percent to one named revision")
...
function revisionDigest(revision, expectedId, revisionName, appName) {
  ...
  if (own(object(revision, "revision projection"), "trafficWeight"))
    revisionKeys.push("trafficWeight")
  exactKeys(revision, revisionKeys, "revision projection")
  if (
    typeof revision.id !== "string" ||
    revision.id.toLowerCase() !== expectedId.toLowerCase() ||
    revision.name !== revisionName
  )
    fail("revision identity mismatch")
  if (
    revision.active !== true ||
    revision.healthState !== "Healthy" ||
    revision.provisioningState !== "Provisioned" ||
    !["Running", "ScaledToZero"].includes(revision.runningState)
  )
    fail("routed revision is not active, healthy, provisioned, and viable")
  if (
    typeof revision.fqdn !== "string" ||
    !fqdnPattern.test(revision.fqdn) ||
    revision.fqdn.includes("..")
  )
    fail("routed revision FQDN is invalid")
  ...
  if (!Array.isArray(template.containers) || template.containers.length !== 1)
    fail("revision must contain exactly one container")
  const container = object(template.containers[0], "revision container")
  if (container.name !== appName)
    fail("revision does not contain the expected container")
  const imagePrefix = `${IMAGE_REPOSITORY}@`
  if (
    typeof container.image !== "string" ||
    !container.image.startsWith(imagePrefix) ||
    !digestPattern.test(container.image.slice(imagePrefix.length))
  )
    fail("routed image is not the expected immutable digest")
  if (!Array.isArray(container.env)) fail("revision environment is malformed")
  const modes = container.env.filter(
    (entry) =>
      entry &&
      typeof entry === "object" &&
      !Array.isArray(entry) &&
      entry.name === "ARTICLE_STORAGE_SCHEMA_MODE",
  )
  if (
    modes.length !== 1 ||
    modes[0].value !== "migrate" ||
    own(modes[0], "secretRef")
  )
    fail("schema mode must be one literal migrate value")
...
function discoverAzureDigest(config, environment) {
  ...
  exactKeys(
    ...
    "Container App projection",
  )
  if (
    typeof app.id !== "string" ||
    app.id.toLowerCase() !== appPath.toLowerCase() ||
    app.name !== config.containerAppName
  )
    fail("Container App identity mismatch")
  if (
    app.provisioningState !== "Succeeded" ||
    app.activeRevisionsMode !== "Multiple" ||
    app.external !== true ||
    app.allowInsecure !== false
  )
    fail("Container App mode, provisioning, or ingress is unsafe")
  if (
    typeof app.location !== "string" ||
    !app.location ||
    typeof app.fqdn !== "string" ||
    !fqdnPattern.test(app.fqdn) ||
    app.fqdn.includes("..")
  )
    fail("Container App location or FQDN is malformed")
  const appTemplate = object(app.template, "Container App template")
  if (
    !Array.isArray(appTemplate.containers) ||
    appTemplate.containers.length !== 1
  )
    fail("Container App template must contain exactly one container")
  const appContainer = object(
    appTemplate.containers[0],
    "Container App container",
  )
  if (
    typeof appContainer.name !== "string" ||
    !safeRevisionPattern.test(appContainer.name)
  )
    fail("Container App container name is missing or unsafe")
...
function ghGet(endpoint, label, environment) {
  return parseJson(
    executeCommand("gh", ghArgs(endpoint, "GET"), label, environment),
    label,
  )
}
...
function validatePackage(value) {
  ...
  if (
    ...
    fail("GitHub package identity, type, or visibility is unsafe")
}
...
function validateVersions(rawVersions, routedDigest, now) {
  ...
  for (const raw of rawVersions) {
    ...
    if (
      ...
      fail("GitHub package version ID is invalid or duplicated")
    if (
      typeof version.name !== "string" ||
      !digestPattern.test(version.name) ||
      digests.has(version.name)
    )
      fail("GitHub package version digest is invalid or duplicated")
    ...
    if (metadata.package_type !== "container" || !Array.isArray(container.tags))
      fail("GitHub container metadata is malformed")
    const tags = new Set()
    for (const tag of container.tags) {
      if (typeof tag !== "string" || !tagPattern.test(tag) || tags.has(tag))
        fail("GitHub package version tag is mutable, unknown, or duplicated")
      tags.add(tag)
    }
    ids.add(version.id)
  ...
  if (versions.filter((version) => version.name === routedDigest).length !== 1)
    fail("routed digest is not present exactly once in GHCR")
  return versions
}
...
function readGithubSnapshot(routedDigest, now, environment) {
  ...
  for (let page = 1; page <= MAX_PAGES; page += 1) {
    ...
    if (rawVersions.length > MAX_VERSIONS)
      fail("GitHub version count exceeds 10000")
    if (values.length < 100)
      return validateVersions(rawVersions, routedDigest, now)
...
function snapshotFingerprint(versions) {
  const canonical = versions
    .map((version) => ({
      ...
      updated_at: version.updated_at,
      metadata: {
        package_type: version.metadata.package_type,
        container: { tags: [...version.metadata.container.tags].sort() },
      },
    }))
...
function printPlan(mode, versions, candidates) {
  ...
  for (const candidate of candidates)
    writeOutput(
      `candidate id=${candidate.id} digest=${candidate.name} created_at=${candidate.created_at}\n`,
    )
}
...
export function main(
  ...
  runner = defaultRun,
  writer = (value) => process.stdout.write(value),
) {
  ...
  try {
    ...
    if (candidates.length > MAX_DELETIONS)
      fail("planned deletion count exceeds 100")
    ...
    if (
      preDeleteDigest !== routedDigest ||
      snapshotFingerprint(preDelete) !== snapshotFingerprint(initial)
    )
      fail("Azure or GHCR state changed before deletion; zero versions deleted")
    let deleted = 0
    for (const candidate of candidates) {
      try {
        ...
        fail(
          `delete failed for failed ID ${candidate.id}; ${deleted} already deleted; cleanup needs operator review`,
        )
      }
    }
    try {
      ...
      if (snapshotFingerprint(finalSnapshot) !== snapshotFingerprint(expected))
        fail("final GHCR snapshot differs from the planned result")
    } catch {
...
if (invokedPath === import.meta.url) {
  try {
    ...
    const message =
      error instanceof SafeError ? error.message : "unexpected internal error"
